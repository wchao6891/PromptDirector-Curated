...
for (const catalogName of ["catalog.json", "public-catalog.json"]) {
  const catalogUrl = new URL(`../site/${catalogName}`, import.meta.url);
  const catalog = normalizeSiteCatalog(JSON.parse(await readFile(catalogUrl, "utf8")));
  if (catalogName === "catalog.json" && catalog.themes.some((theme) => theme.rightsStatus === "source_unverified")) {
    throw new Error("已发布插件目录不支持未核验授权状态，请使用公共网页目录");
  }
